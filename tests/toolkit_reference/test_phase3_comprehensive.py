...
class Phase3TestSuite:
    ...
    async def test_phase3_orchestrator_basic(self):
        ...
        try:
            from ctf_core.phase3_orchestrator import SecurityOrchestrator
    ...
    async def test_phase3_orchestrator_phases(self):
        ...
        try:
            from ctf_core.phase3_orchestrator import SecurityOrchestrator, OrchestrationPhase
    ...
    async def test_phase3_coordination_basic(self):
        ...
        try:
            from ctf_core.phase3_coordination import Phase3TaskOrchestrator, AgentRole, TaskPriority
            ...
            # Test task creation
            from ctf_core.phase3_coordination import Phase3Task
            task = Phase3Task(
                task_id="",
    ...
    async def test_phase3_coordination_task_execution(self):
        ...
        try:
            from ctf_core.phase3_coordination import Phase3TaskOrchestrator, AgentRole, TaskPriority, Phase3Task
    ...
    async def test_phase3_mcp_integration(self):
        """Test Phase 3 MCP integration"""
        print("Testing Phase 3 MCP integration...")
        start = time.time()
        
        try:
            from ctf_core.phase3_mcp import Phase3MCPServer
            
            server = Phase3MCPServer(str(self.workspace_path))
            
            # Test server creation
            assert server is not None
            assert server.workspace_path.exists()
            ...
            assert server_info['name'] == "CTF Toolkit Phase 3"
            assert server_info['version'] == "3.0.0"
            
            # Test session creation
            session_data = await server.handle_create_session({
                "challenge_type": "web",
                "target": "192.168.1.100",
                "objectives": ["Find vulnerabilities", "Exploit SQL injection"]
            })
            
            assert session_data is not None
            assert 'session_id' in session_data
            assert session_data['challenge_type'] == "web"
            
            duration = time.time() - start
            self.log_test("MCP Integration", True, duration=duration,
                         details=f"Registered {len(server.tools)} MCP tools")
            
        except Exception as e:
            duration = time.time() - start
            self.log_test("MCP Integration", False, str(e), duration)
    ...
    async def test_phase3_performance_stress(self):
        ...
        try:
            from ctf_core.phase3_coordination import Phase3TaskOrchestrator, Phase3Task, AgentRole, TaskPriority
    ...
    def run_all_phase3_tests(self):
        ...
        tests = [
            ...
            self.test_phase3_mcp_integration,
